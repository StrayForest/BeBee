...
local function query(name)
    return html5.run(
        "new URLSearchParams(window.location.search).get(" .. json.encode(name) .. ") || ''"
    )
end
...
function M.run()
    ...
    local ok, probe_result = pcall(function()
        ...
        elseif scenario == "save" then
            return service.save({ save_version = 1, marker = marker })
        ...
        elseif scenario == "quick_checkpoints" then
            ...
            local first = service.save({ save_version = 1, marker = marker .. "-1" })
            if not first.ok then return first end
            local second = service.save({ save_version = 1, marker = marker .. "-2" })
            if not second.ok then return second end
            local third = service.save({ save_version = 1, marker = marker .. "-3" })
            if not third.ok then return third end
            return service.load()
        elseif scenario == "corrupt_newest" then
            ...
            local first = service.save({ save_version = 1, marker = marker .. "-stable" })
            if not first.ok then return first end
            local second = service.save({ save_version = 1, marker = marker .. "-newest" })
            if not second.ok then return second end
    ...
    if not ok then
        publish({
            ...
            error = tostring(probe_result),
        })
        return
    end

    publish(result_payload(scenario, probe_result, {
