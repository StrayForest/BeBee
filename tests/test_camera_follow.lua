...
return {
    ...
    cases = {
        ...
        {
            name = "normal camera reaches Golden Fields and clamps at expanded edge",
            run = function()
                local state = follow.new()
                for _ = 1, 360 do follow.step(state, 4480, 900, 1/60, false) end
                t.assert_true(near(3920, state.x, 0.01))
                t.assert_true(math.abs(900 - state.y) <= 48.01)
                t.assert_true(math.abs(4480 - state.x) <= 640)
            end,
        },
        {
            name = "reduced motion removes follow lag while preserving expanded bounds",
            run = function()
                local state = follow.new()
                follow.step(state, 1700, 1000, 1/60, true)
                t.assert_equal(1700, state.x)
                t.assert_equal(1000, state.y)
                follow.step(state, 4560, 1600, 1/60, true)
                t.assert_equal(3920, state.x)
