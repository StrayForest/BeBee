...
local function rosewood_completion_activates_alpine_bloom()
    local save = progression.new_save()
    for index = 1, 20 do
        save.world.campaign_completion[catalog.patches[index].id] = true
    end
    test.assert_equal("region_05", region.active_id(save))
    local summary = region.active_summary(save)
    test.assert_equal("region_05", summary.id)
    test.assert_equal(4, summary.total)
    test.assert_equal(0, summary.restored_count)
    test.assert_equal("r05_m01", summary.next_meadow_id)
    test.assert_equal("RESTORE SNOWDROP PASS · 0/4", region.active_objective_text(save))
    test.assert_equal("region_05", region.region_id_for_meadow("r05_m03"))
end

local function alpine_bloom_is_content_chain_not_new_core_system()
    local save = progression.new_save()
    for index = 1, 20 do
        save.world.campaign_completion[catalog.patches[index].id] = true
    end
    save.player.upgrades.upgrade_buzz = 3
    local first = catalog.patches[21]
    test.assert_equal("r04_m04_patch_01", first.requires_patch_id)
    test.assert_equal("flower_edelweiss", first.flower_id)
    test.assert_equal(3, first.requires_buzz_level)
    local eligible, reason = progression.patch_eligibility(save, first)
    test.assert_true(eligible, tostring(reason))
end
