...
local function migration_failure_preserves_original_slot_on_next_save()
    local state = {
        slots = {
            a = {
                format_version = 1,
                generation = 9,
                payload = { save_version = -1, marker = "unmigratable-original" },
            },
        },
    }
    ...
    local saved = adapter.save({ save_version = 1, marker = "fresh" })
    ...
    local original_after = table_utils.canonical_encode(state.slots.a)
    local original_before = table_utils.canonical_encode(original)
    test.assert_equal(original_before, original_after)
end
...
local function durability_values_follow_contract()
    ...
    local saved = adapter.save({ save_version = 1, marker = "saved" })
    test.assert_true(saved.ok)
...
return {
    ...
    cases = {
        {
            name = "migration_failure_preserves_original_slot_on_next_save",
            run = migration_failure_preserves_original_slot_on_next_save,
        },
        {
            name = "durability_values_follow_contract",
            run = durability_values_follow_contract,
        },
    },
}
