...
local function fake_backend()
    local state = {
        slots = {}, corrupt = {}, fail_load = {}, fail_save = {},
        corrupt_after_save = {}, fail_delete = {}, measured_bytes = 128,
    }
    local backend = {}

    function backend.load(slot)
        if state.fail_load[slot] then return { ok = false, code = state.fail_load[slot], error = "forced load failure" } end
        if state.corrupt[slot] then return { ok = false, code = "load_error", corrupt = true, error = "forced corruption" } end
    ...
    function backend.save(slot, value)
        if state.fail_save[slot] then return { ok = false, code = "write_error", error = "forced write failure" } end
        state.slots[slot] = table_utils.deep_copy(value)
        if state.corrupt_after_save[slot] then state.corrupt[slot] = true else state.corrupt[slot] = nil end
        return { ok = true }
    end

    function backend.measure(_value) return { ok = true, bytes = state.measured_bytes } end
    function backend.delete(slot)
        if state.fail_delete[slot] then return { ok = false, code = "delete_error", error = "forced delete failure" } end
...
local function payload(marker) return { save_version = 2, marker = marker } end
...
local function newest_generation_wins()
    local adapter = adapter_with_fake()
    adapter.save(payload("one")); adapter.save(payload("two"))
...
local function corrupt_a_recovers_b()
    local adapter, state = adapter_with_fake()
    adapter.save(payload("one")); adapter.save(payload("two")); state.corrupt.a = true
    local loaded = adapter.load()
    test.assert_true(loaded.ok)
...
local function corrupt_b_recovers_a()
    local adapter, state = adapter_with_fake()
    adapter.save(payload("one")); adapter.save(payload("two")); state.corrupt.b = true
    local loaded = adapter.load()
    test.assert_true(loaded.ok)
...
local function missing_peer_recovers_valid_slot()
    local adapter, state = adapter_with_fake()
    state.slots.b = { format_version = 1, generation = 7, payload = payload("kept") }
    local loaded = adapter.load()
    test.assert_true(loaded.ok)
    test.assert_equal("recovered_single_valid_slot", loaded.code)
...
local function both_invalid_fail_without_throwing()
    local adapter, state = adapter_with_fake()
    state.corrupt.a = true; state.corrupt.b = true
    local loaded = adapter.load()
...
local function equal_generation_identical_is_ok()
    ...
    state.slots.a = table_utils.deep_copy(envelope); state.slots.b = table_utils.deep_copy(envelope)
    local loaded = adapter.load()
    test.assert_true(loaded.ok)
...
local function failed_write_preserves_previous_generation()
    local adapter, state = adapter_with_fake()
    adapter.save(payload("stable")); state.fail_save.b = true
    local failed = adapter.save(payload("new"))
    test.assert_false(failed.ok); test.assert_equal("write_error", failed.code)
    state.fail_save.b = nil
    local loaded = adapter.load()
    test.assert_true(loaded.ok); test.assert_equal("stable", loaded.value.marker); test.assert_equal(1, loaded.diagnostics.last_generation)
end

local function failed_readback_preserves_previous_generation()
    local adapter, state = adapter_with_fake()
    adapter.save(payload("stable")); state.corrupt_after_save.b = true
    local failed = adapter.save(payload("new"))
    test.assert_false(failed.ok); test.assert_equal("readback_error", failed.code)
    local loaded = adapter.load()
    test.assert_true(loaded.ok); test.assert_equal("stable", loaded.value.marker); test.assert_true(loaded.recovery)
end
...
local function migration_fixture_upgrades_v0()
    ...
    local loaded = adapter.load()
    test.assert_true(loaded.ok)
    test.assert_equal(2, loaded.value.save_version)
    test.assert_equal("legacy", loaded.value.marker)
end

local function migration_fixture_upgrades_p2_v1_without_losing_progress()
    local adapter, state = adapter_with_fake()
    state.slots.a = {
        format_version = 1, generation = 3,
        payload = {
            save_version = 1,
            player = { honey = 45 },
            world = { campaign_completion = { r01_m01_patch_01 = true } },
        },
    }
    local loaded = adapter.load()
    test.assert_true(loaded.ok)
...
local function newer_runtime_version_is_rejected()
    local adapter, state = adapter_with_fake()
    state.slots.a = { format_version = 1, generation = 1, payload = { save_version = 99, marker = "future" } }
    local loaded = adapter.load()
    test.assert_false(loaded.ok); test.assert_equal("both_slots_invalid", loaded.code)
end

local function warning_size_is_reported_but_saved()
    local adapter, state = adapter_with_fake(); state.measured_bytes = 131072
    local saved = adapter.save(payload("warning"))
    test.assert_true(saved.ok); test.assert_true(saved.diagnostics.size_warning); test.assert_equal(131072, saved.diagnostics.serialized_size_bytes)
end

local function release_gate_blocks_write()
    local adapter, state = adapter_with_fake(); state.measured_bytes = 262144
    local saved = adapter.save(payload("too-large"))
    test.assert_false(saved.ok); test.assert_equal("size_release_gate_exceeded", saved.code)
    test.assert_equal(nil, state.slots.a); test.assert_equal(nil, state.slots.b)
end

local function storage_unavailable_is_explicit()
    local adapter, state = adapter_with_fake(); state.fail_load.a = "storage_unavailable"
    local loaded = adapter.load()
    test.assert_false(loaded.ok); test.assert_equal("storage_unavailable", loaded.code)
end

local function has_reflects_valid_save()
    local adapter = adapter_with_fake()
    local before = adapter.has(); test.assert_true(before.ok); test.assert_false(before.value)
    adapter.save(payload("present"))
    local after = adapter.has(); test.assert_true(after.ok); test.assert_true(after.value)
end

local function partial_delete_is_explicit()
    local adapter, state = adapter_with_fake()
    adapter.save(payload("one")); adapter.save(payload("two")); state.fail_delete.b = true
    local deleted = adapter.delete()
    test.assert_false(deleted.ok); test.assert_equal("delete_partial_failure", deleted.code)
    test.assert_equal(nil, state.slots.a); test.assert_true(state.slots.b ~= nil)
end
