#!/usr/bin/env python3
"""Exercise BeBee movement plus the P2 pollination extension in real Chromium."""
...
def wait_ready(page: Page, *, head_sha: str, state_id: str, timeout_ms: int) -> dict[str, object]:
    page.wait_for_function(
        "() => window.__bebeeQA && window.__bebeeQA.captureReady === true && !!window.__bebeeMovementQA",
        timeout=timeout_ms,
    )
    qa = page.evaluate("() => structuredClone(window.__bebeeQA)")
    if qa.get("stateId") != state_id or qa.get("buildCommitSha") != head_sha:
        raise RuntimeError(f"movement QA provenance mismatch: {qa!r}")
    return bridge(page)


def assert_no_errors(console_errors: list[str], page_errors: list[str], label: str) -> None:
    if console_errors or page_errors:
        raise RuntimeError(f"{label}: console={console_errors!r} page={page_errors!r}")
...
def record_desktop_motion(browser: Browser, *, base_url: str, head_sha: str, output_root: Path, timeout_ms: int) -> dict[str, object]:
    frames = output_root / "movement_dense" / "desktop_reference_frames"
    ...
    with tempfile.TemporaryDirectory(prefix="bebee-movement-desktop-") as video_dir:
        context = browser.new_context(
            viewport={"width": 1280, "height": 720}, screen={"width": 1280, "height": 720},
            device_scale_factor=1, record_video_dir=video_dir, record_video_size={"width": 1280, "height": 720},
        )
        ...
        try:
            ...
            screenshot(page, frames / "00-idle.png")
            t0 = time.monotonic()
            page.keyboard.down("d")
            page.wait_for_timeout(350)
            early = bridge(page)
            screenshot(page, frames / "01-accelerating.png")
            page.wait_for_timeout(500)
            cruise = bridge(page)
            screenshot(page, frames / "02-cruise.png")
            page.keyboard.down("w")
            page.wait_for_timeout(650)
            diagonal = bridge(page)
            screenshot(page, frames / "03-diagonal.png")
            page.keyboard.up("w")
            page.keyboard.up("d")
            page.wait_for_timeout(450)
            stopped = bridge(page)
            screenshot(page, frames / "04-stopped.png")
            duration = time.monotonic() - t0

            if early.get("inputSource") != "keyboard" or float(early["speed"]) <= 100:
                raise RuntimeError(f"desktop keyboard did not accelerate: {early!r}")
            if float(cruise["beeX"]) - float(start["beeX"]) <= 80:
                raise RuntimeError(f"desktop displacement too small: start={start!r} cruise={cruise!r}")
            if not 250 <= float(cruise["speed"]) <= 305:
                raise RuntimeError(f"desktop cruise speed outside band: {cruise!r}")
            if diagonal.get("inputSource") != "keyboard" or abs(math.hypot(float(diagonal["intentX"]), float(diagonal["intentY"])) - 1) > 0.02:
                raise RuntimeError(f"desktop diagonal intent invalid: {diagonal!r}")
            if float(stopped["speed"]) > 5 or stopped.get("inputSource") != "none":
                raise RuntimeError(f"desktop release did not settle: {stopped!r}")
            if int(stopped.get("boundHits", 0)) != 0:
                raise RuntimeError(f"desktop central proof hit bounds: {stopped!r}")
            observed_frames = int(stopped["frame"]) - int(start["frame"])
            observed_fps = observed_frames / max(duration, 0.001)
            if observed_fps < 20 or not 2.0 <= duration <= 6.0:
                raise RuntimeError(f"desktop runtime timing invalid: fps={observed_fps:.2f} duration={duration:.3f}")
            assert_no_errors(console_errors, page_errors, "desktop movement")
            result = {
                "viewport": {"id": "desktop_reference", "width": 1280, "height": 720},
                "exercise_seconds": round(duration, 3), "observed_frames": observed_frames,
                "observed_fps": round(observed_fps, 2), "start": start, "early": early,
                "cruise": cruise, "diagonal": diagonal, "stopped": stopped,
                "console_error_count": len(console_errors), "page_error_count": len(page_errors),
            }
        finally:
            context.close()
        if video is None:
            raise RuntimeError("desktop movement video handle missing")
        video.save_as(str(video_path))
    if not video_path.is_file() or video_path.stat().st_size == 0:
        raise RuntimeError("desktop movement video missing")
...
def record_touch_motion(browser: Browser, *, base_url: str, head_sha: str, output_root: Path, timeout_ms: int) -> dict[str, object]:
    frames = output_root / "movement_dense" / "mobile_landscape_frames"
    ...
    with tempfile.TemporaryDirectory(prefix="bebee-movement-touch-") as video_dir:
        context = browser.new_context(
            ...
            record_video_dir=video_dir, record_video_size={"width": 844, "height": 390},
        )
        page = context.new_page()
        video = page.video
        console_errors, page_errors = install_error_capture(page)
        ...
        try:
            ...
            page.wait_for_timeout(500)
            active = bridge(page)
            screenshot(page, frames / "01-horizontal.png")
            page.wait_for_timeout(400)
            dispatch_touch(session, "touchMove", [diagonal_point])
            page.wait_for_timeout(500)
            diagonal = bridge(page)
            screenshot(page, frames / "02-diagonal.png")
            dispatch_touch(session, "touchEnd", [])
            page.wait_for_timeout(500)
            stopped = bridge(page)
            screenshot(page, frames / "03-stopped.png")
            duration = time.monotonic() - t0
            if active.get("inputSource") != "touch" or float(active["speed"]) <= 100:
                raise RuntimeError(f"touch movement did not activate: {active!r}")
            if float(active["beeX"]) - float(start["beeX"]) <= 50:
                raise RuntimeError(f"touch displacement too small: {active!r}")
            ...
            if float(stopped["speed"]) > 5 or stopped.get("inputSource") != "none":
                raise RuntimeError(f"touch release did not settle: {stopped!r}")
            assert_no_errors(console_errors, page_errors, "touch movement")
            result = {
                "viewport": {"id": "mobile_landscape", "width": 844, "height": 390},
                "exercise_seconds": round(duration, 3), "start": start, "active": active,
                "diagonal": diagonal, "stopped": stopped,
                "console_error_count": len(console_errors), "page_error_count": len(page_errors),
            }
        finally:
            context.close()
        if video is None:
            raise RuntimeError("touch movement video handle missing")
        video.save_as(str(video_path))
    if not video_path.is_file() or video_path.stat().st_size == 0:
        raise RuntimeError("touch movement video missing")
    result["video_file"] = video_path.relative_to(output_root).as_posix()
    result["video_sha256"] = sha256_file(video_path)
    result["frame_files"] = [p.relative_to(output_root).as_posix() for p in sorted(frames.glob("*.png"))]
    return result
...
def verify_modal_and_reduced_motion(browser: Browser, *, base_url: str, head_sha: str, timeout_ms: int) -> dict[str, object]:
    context = browser.new_context(viewport={"width": 1280, "height": 720}, screen={"width": 1280, "height": 720}, device_scale_factor=1)
    page = context.new_page()
    session = context.new_cdp_session(page)
    console_errors, page_errors = install_error_capture(page)
    ...
    try:
        page.goto(with_query(base_url, qa="movement_empty", qa_seed=88008, reduced_motion=1), wait_until="load", timeout=timeout_ms)
        ...
        if start.get("reducedMotion") is not True:
            raise RuntimeError(f"reduced motion override missing: {start!r}")
        page.keyboard.down("d")
        page.wait_for_timeout(700)
        moving = bridge(page)
        page.keyboard.up("d")
        page.wait_for_timeout(300)
        lag_x = abs(float(moving["beeX"]) - float(moving["cameraX"]))
        lag_y = abs(float(moving["beeY"]) - float(moving["cameraY"]))
        if lag_x > 0.1 or lag_y > 0.1:
            raise RuntimeError(f"reduced-motion camera lagged: {moving!r}")
        ...
        displacement = abs(float(after_modal["beeX"]) - float(before_modal["beeX"]))
        if displacement > 1.0:
            raise RuntimeError(f"movement leaked through modal: {before_modal!r} -> {after_modal!r}")
        dispatch_escape(session, page)
        ...
        return {
            "reduced_motion_applied": True, "camera_lag_abs_x": round(lag_x, 4),
            "camera_lag_abs_y": round(lag_y, 4), "modal_displacement": round(displacement, 4),
            "modal_focus_consumed_movement": True, "console_error_count": len(console_errors),
...
def main() -> int:
    ...
    print(
        "P1 movement + P2 pollination browser proof: PASS "
...
if __name__ == "__main__":
    try:
        ...
        print(f"Movement/P2 browser proof failed: {exc}")
        raise SystemExit(1)
